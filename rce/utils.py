...
class ConstraintLearning(object):
    ...
    def constraint_extrapolation_iai(self, class_c):
        ...
        for leaf in leaf_nodes:
            ...
            while not stop:
                ...
                constraint['threshold'] = threshold-0.00001 if sign else -threshold
    ...
    def constraint_extrapolation_skTree(self, class_c):
        ...
        if class_c == 'multiclass':
            ...
            columns = ['ID', 'node_ID'] + [feature for feature in self.get_features_list()] + ['threshold'] + columns_classes
        else:
            columns = ['ID', 'node_ID'] + [feature for feature in self.get_features_list()] + ['threshold', 'prediction']
        constraints = pd.DataFrame(columns=columns)
